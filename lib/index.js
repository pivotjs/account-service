...
var AuthenticationService = (function () {
    ...
    AuthenticationService.prototype.signin = function (email, password, options) {
        ...
        return this.findOne({ email: email })
            ...
            return _this.ensureOutOfFailedAttemptsDelay(account)
                .then(function () { return _this.ensureSamePassword(account, password); })
    ...
    AuthenticationService.prototype.ensureOutOfFailedAttemptsDelay = function (account) {
        var reachedMaxFailedAttempts = account.failed_attempts >= this.config.maxFailedAttempts;
        var inFailedAttemptsDelay = new Date().getTime() < (account.failed_attempt_at + this.config.delayOnMaxFailedAttempts);
        if (reachedMaxFailedAttempts && inFailedAttemptsDelay) {
            return Promise.reject(exports.AuthenticationErrors.FAILED_ATTEMPTS_DELAY);
        }
        else {
            return Promise.resolve(account);
        }
